	...
	public void run() {
		...
		try {
			...
			while(true) {
				...
				if(text == null) {
					System.out.println(m_ID + "이 나갔습니다.");
					for(int i=0; i<ChatServer.m_OutputList.size(); ++i)
					{
						ChatServer.m_OutputList.get(i).println(m_ID + " is Out.");
						ChatServer.m_OutputList.get(i).flush();
					}
					break;
				}
				String[] split = text.split("yeeunComp");
				if(split.length == 2 && split[0].equals("ID"))
				{
					m_ID = split[1];
					System.out.println(m_ID + "이 입장했습니다");
					for(int i=0; i< ChatServer.m_OutputList.size(); ++i)
					{
						ChatServer.m_OutputList.get(i).println(m_ID + " 이 들어왔습니다.");
						ChatServer.m_OutputList.get(i).flush();
					}
					continue;
				}
