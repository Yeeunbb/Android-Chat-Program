...
public class CustomAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if ( true ) {
            ...
            convertView = inflater.inflate(R.layout.left_row, parent, false);
        ...
        if( m_List.get(position).type == 0 ) {
            text.setBackgroundResource(android.R.color.holo_blue_dark);
            layout.setGravity(Gravity.LEFT);
            viewRight.setVisibility(View.GONE);
            viewLeft.setVisibility(View.GONE);
        }else if(m_List.get(position).type == 1){
            text.setBackgroundResource(android.R.color.holo_orange_dark);
